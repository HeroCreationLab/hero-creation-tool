...
- Compatibility update to `Foundry v11.xyz` & `dnd5e 3.1.2`.
...
- Skill selectors now show the ability that governs the skill [(#77)](https://github.com/HeroCreationLab/hero-creation-tool/issues/77)
...
- Added module setting to trim subclass names - _for DDB Importer subclasses like "Assassin (Rogue)" -> "Assassin"_
...
- Styled clickable icons _-the ones that open a popup window-_ with a box shadow on hover, to highlight their clickability. Affects avatar, token, any race/class feature (but not the proper Race/Class icon), feat, background feature, equipment, spells.
...
- Updated module to work with Foundry VTT v9 _(from this version onwards, the module will likely no longer be retrocompatible with version 0.8.x)_
